...
function parseXY(config, _chartProps, callback, parseOpts) {
	// Build chart settings from defaults or provide settings

	parseOpts = parseOpts || {};
	// Clone this so that we aren't modifying the original
		...
		if (_computed.count > 0) {
			...
			if (chartProps.mobile) {
				if (chartProps.mobile.scale) {
					...
					if (currMobile) {
						var domain = help.coumputeScaleDomain(currMobile, _computed.data, {
							nice: true,
							minZero: _computed.hasColumn
						});
	...
	// create the data structure for the renderer based on input
	if (bySeries.hasDate) {
	...
	if (bySeries.isNumeric) {
		...
		each(currScale.tickValues, function(v) {
			var tickPrecision = help.precision(Math.round(v*factor)/factor);
			if (tickPrecision > currScale.tickPrecision) {
				currScale.precision = tickPrecision;
			}
		});
