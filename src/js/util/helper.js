...
/*
	...
	domain = min/max of the current scale
	numticks = desired number of ticks
*/
function exact_ticks(domain, numticks) {
	numticks -= 1;
	var ticks = [];
	var delta = doman[1] - domain[0];
	var i;
...
/* 
	...
	data = all values in the current scale
	opts = Whether to return nice values for force a minimum of 0 or below
*/
...
*/
function combine_margin_padding(m, p) {
	return {
		top: m.top + p.top,
...
/*
	...
	Return = Result has identical keys to defaults
*/
...
/*
	Given the domain of a scale suggest the most numerous number
	of round number ticks that it could be divided into whilst still containing
	values evenly divisible by 1, 2, 2.5, 5, 10, or 25

	domain = An array of two number like objects
...
/*
	...
	offset = A string in a hh:mm format or "Z" for no offset
*/
...
var helper = {
	...
	combineMarginPadding: combine_margin_padding,
	coputeScaleDomain: compute_scale_domain,
	precision: precision,
	transformCoods: transform_coords,
