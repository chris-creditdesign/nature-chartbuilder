...
function parseDelimInput(input, opts) {
	...
	if (index_types.length !== 1 && !_defaultOpts.type) {
		// there is a possibly more than one type of data, an error will be thrown in validate-date-input
	} else {
		...
		if (isNumeric && !_defaultOpts.type && _defaultOpts.checkForDate) {
			// if the entries are certain four digit numbers that look like years
			// reparse as years if there isn't a specific type
